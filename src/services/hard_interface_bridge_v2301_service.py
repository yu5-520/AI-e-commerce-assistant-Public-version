"""V22.3.0.1 non-Agent bridge for the hard-interface runtime.

The bridge preserves three deterministic responsibilities that were previously bundled
with the retired V22.2.6 Agent runtime overlay:

* evidence-based product signal admission;
* immediate Agent input Artifact compilation at producer handoff;
* layered pipeline/status read models.

It never replaces Agent1, Agent2, Token Runtime or the hard pipeline tick. V23.1.4 makes
the producer-side Agent1 projection use the same V22.5.8 transport as active execution.
"""
...
def bind_hard_interface_bridge_v2301() -> Dict[str, Any]:
    """Bind producer handoffs and read models without installing an Agent runtime."""
    global _BOUND
    if _BOUND:
    ...
    admission.product_signal_admission_station_v225 = flow.product_signal_admission_station_v226
    admission.ARTIFACT_SIGNAL_ADMISSION_VERSION = flow.SIGNAL_ADMISSION_POLICY_VERSION
...
def hard_interface_bridge_status(*, idempotent: bool = False) -> Dict[str, Any]:
    return {
        ...
        "signalAdmissionOwner": "end_to_end_agent_flow_v226.evidence_admission_only",
