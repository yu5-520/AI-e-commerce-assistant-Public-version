"""V22.5.7 release-bound single three-Agent station worker."""
...
from src.services.agent_runtime_hard_interface_v2257_service import (
...
STATION_AGENT_WORKER_VERSION = "22.5.7"
...
def worker_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "agent1InputProjectionVersion": "22.5.7",
        ...
        "dataVersionSelection": "oldest_highest_priority_runnable_v2257",
        "forceNewSnapshot": False,
        "agentExecutionMode": "semantic_continuity_then_execution_lock",
        "agent1RuntimeSource": "artifactRefs.agent1InputRef.v2",
...
def _run_one(worker_id: str, config: Dict[str, Any]) -> Dict[str, Any]:
    if bool(config.get("agentPipelineEnabled")):
        ...
        if agent_result.get("ran"):
            ...
            return {"type": "three_agent_hard_interface_worker", "ran": True, "result": agent_result}
    station_result = run_next_station_job(worker_id=worker_id, system_type=str(config["systemType"]))
...
def worker_status(include_queue: bool = True) -> Dict[str, Any]:
    ...
    if include_queue:
        ...
        try:
            ...
            selected = select_runnable_data_version_v225(state.get("lastSelectedDataVersion"))
...
def start_station_queue_worker(worker_id: str = "fastapi-three-agent-worker") -> Dict[str, Any]:
    global _THREAD
    ...
    _THREAD = threading.Thread(
        target=_loop,
        args=(worker_id,),
        name="station-agent-v2257-worker",
        daemon=True,
    )
    _THREAD.start()
    time.sleep(0.05)
...
def run_worker_tick(worker_id: str = "manual-three-agent-tick", limit: int | None = None) -> Dict[str, Any]:
    config = worker_config()
