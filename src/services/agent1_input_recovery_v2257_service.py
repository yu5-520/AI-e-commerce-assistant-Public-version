"""V22.5.7 targeted recovery for observations produced from legacy Agent1 inputs.

Only legal native observations whose current ``agent1InputRef`` is not the v2 semantic
projection are eligible. The immutable signalRef is preserved, old Agent1/downstream
refs are removed, and the item returns once to ``agent1_pending``. Re-running the
recovery is idempotent because updated rows no longer match ``observed_soft_gate``.
"""
...
from src.services.agent_input_contract_v2257_service import AGENT1_INPUT_SCHEMA
...
def _native_observation(row: Dict[str, Any]) -> bool:
    payload = _load(row.get("payload"))
    reasons = {
        str(obj.get("diagnosticHoldReason") or "")
        for obj in _walk(payload)
        if obj.get("diagnosticHoldReason")
    }
    decisions = {
        str(obj.get("decisionType") or "").lower()
        for obj in _walk(payload)
        if obj.get("decisionType")
    }
    return "native_observation" in reasons or (
        "observe" in decisions
        and str(row.get("current_stage") or "") == "observed_soft_gate"
        and str(row.get("status") or "") == "observed"
    )
...
def _input_schema(refs: Dict[str, Any]) -> str:
    ...
    try:
        ...
        return "unresolved"
    return str(_dict(value).get("schema") or "unknown")


def _eligible(row: Dict[str, Any], refs: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    if str(row.get("current_stage") or "") != "observed_soft_gate":
        return False, {"reason": "stage_not_observed"}
    if str(row.get("status") or "") != "observed":
        return False, {"reason": "status_not_observed"}
    if not _native_observation(row):
        return False, {"reason": "not_native_observation"}
    ...
    if schema == AGENT1_INPUT_SCHEMA:
        return False, {"reason": "already_semantic_v2", "inputSchema": schema}
    return True, {
        "reason": "legacy_agent1_input_requires_semantic_rejudgment",
        "inputSchema": schema,
...
def requeue_legacy_agent1_observations_v2257(
    *,
    ...
    clauses = ["current_stage='observed_soft_gate'", "status='observed'"]
    params: list[Any] = []
    if resolved_version:
    ...
    if apply:
        for row, refs, diagnosis in eligible:
            ...
            recovery_payload = {
                ...
                "reason": "agent1_input_semantic_continuity_rejudgment",
            ...
            with connect() as conn:
                conn.execute(
                    ...
                    """,
                    (
                        f"agent1_input_v2257_rejudgment:{row.get('data_version')}:{row.get('item_id')}",
                        dumps(
                            {
                                "version": RECOVERY_VERSION,
            ...
            envelope = {
                ...
                "inputRef": signal_ref,
                "outputRef": f"agent1_input_v2257_rejudgment:{row.get('data_version')}:{row.get('item_id')}",
                "artifactRefs": clean_refs,
            }
            record_pipeline_item_event(
                envelope,
                station_id="agent1_input_recovery_v2257",
                stage="agent1_pending",
                status="ready",
            ...
            updated.append(
                {
                    ...
                    "productId": row.get("product_id"),
                    "storeId": row.get("store_id"),
                    "fromStage": "observed_soft_gate",
                    "toStage": "agent1_pending",
                    "removedLegacyAgent1InputRef": True,
    ...
    return {
        ...
        "matchedObservationCount": len(rows),
        "eligibleCount": len(eligible),
        "updatedCount": len(updated),
        "updated": updated,
        "skipped": skipped[:100],
        "rule": "Only native observations backed by a legacy Agent1 input schema return once to Agent1.",
    }


__all__ = [
    "RECOVERY_VERSION",
    "latest_product_data_version",
    "requeue_legacy_agent1_observations_v2257",
]
