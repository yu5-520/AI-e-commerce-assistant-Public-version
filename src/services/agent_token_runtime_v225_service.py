...
into authority-bearing JudgementGraph / ActionGraph / OperationGraph surfaces; V26.5
adds addressable Node/Edge lineage while the Java/system call topology and historical
ExecutionHash authority remain unchanged.
"""
...
from src.services.v26_node_edge_lineage_service import (
    install_v26_node_edge_lineage,
)

# Install fail-closed identity/field authority first, migrate the business semantic
# seams into V26.2 graphs, then make those graphs addressable in V26.5. Agent3 runtime
# is intentionally imported only after this point so its module-level `core` reference
# observes both V26 prompt/normalization patches.
RUNTIME_CONTRACT_GUARDS = install_runtime_contract_guards()
V26_BUSINESS_GRAPH_BRIDGE = install_v26_business_graph_bridge()
V26_NODE_EDGE_LINEAGE = install_v26_node_edge_lineage()
