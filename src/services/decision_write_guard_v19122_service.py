...
def _family(decision: Dict[str, Any]) -> str:
    ...
    return str(plan.get("selectedActionFamily") or (package.get("lockedActionFamily") or package.get("selectedActionFamilyHint")) or "").strip()
...
def _creative_plan(decision: Dict[str, Any]) -> Dict[str, Any]:
    plan = _plan(decision)
    package = _package(decision)
    for value in [(plan.get("creativeDraft") or plan.get("creativeTestPlan")), (package.get("creativeDraft") or package.get("creativeTestPlan")), package.get("agentCreativePack")]:
...
def _is_creative_missing_task(decision: Dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        or ("creativeDraft" in reason or "creativeTestPlan" in reason)
