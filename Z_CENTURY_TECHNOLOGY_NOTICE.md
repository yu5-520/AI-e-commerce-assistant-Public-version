...
本声明所称的 Z 派生通用技术，主要指 V25 Unified RAG / Knowledge Plane 中体现的通用框架结构，包括但不限于：
...
上述通用结构属于由 **Z-Century（Z世纪）技术体系及其适用权利人**提供并授权本项目使用的技术能力。相关实现被复制、集成、构建、部署或运行于本仓库，**不构成知识产权转让（Integration ≠ Assignment）**。

## AI 电商项目取得的权利

AI-e-commerce-assistant-Public-version 在本授权范围内取得有限、非独占、不可转让的项目使用权，可用于：
...
本项目**不因持有代码副本或运行实现而取得**上述 Z 派生通用框架的：
...
Z-Century 通用技术体系
        ↓ 有限授权
V25 Knowledge Plane / RAG Framework
        ↓ 业务集成
AI 电商 / 智策产品实现
```
...
除非适用权利人通过书面文件明确转让，Z 派生通用框架相关权利不因本项目的集成、比赛提交、部署、复制或商业运行而自动转移。
